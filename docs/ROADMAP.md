...
- **Google Play readiness** *(Tier 1 — gates the tester rollout)* — Google Play is the short-term distribution target. Audit of the Sep 3 release build (2026-09-07) against current Play requirements; each item is a rejection if left as is:
  ...
  5. **Remove `USE_FULL_SCREEN_INTENT`** — Play restricts it to alarm and calling apps. The native assistant path answers without opening the app, so drop the permission, `ensureFullScreenIntentPermission()` in `MainActivity`, and the full-screen intent on the fallback notification in `WakeWordService` (keep the tap-to-open notification).
  6. **Remove `READ_MEDIA_IMAGES`** — photos come through the system photo picker, which needs no permission; the only remaining reference is the permission prompt in `MainActivity.FolderPickerBridge`.
