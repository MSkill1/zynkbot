...
- **Google Play readiness** *(Tier 1 — gates the tester rollout)* — Google Play is the short-term distribution target. Audit of the Sep 3 release build (2026-09-07) against current Play requirements; each item is a rejection if left as is:
  ...
  7. **AI-generated content policy** — apps that generate content with AI must offer an in-app way to report or flag offensive output. Add a "report this reply" control on assistant messages. Confirm the current policy wording before building.
...
- **Use the desktop's local model from the phone** *(v1.0; added 2026-09-07)* — Today the phone can run a GGUF model on-device, or call a cloud API. It cannot use the model already running on the user's desktop at home, which is the obvious local-first choice for anyone who does not want a model on the phone. The sync service already proxies `/api/ollama/*` to the desktop's Ollama on the LAN; turn that into a first-class backend: a "My desktop" entry in the model picker that resolves the paired desktop's address, health-checks it, streams through the existing mTLS channel, and falls back (with a clear message) when the desktop is away. Check what the current CUSTOM_API_URL path already covers before building.
- **In-app "Report a problem"** *(v1.0; added 2026-09-07)* — One button (Settings, and on each reply) that assembles the last few hundred log lines, the build hash, device model, and the current thread into a text block the user copies into a GitHub issue. No server, nothing sent automatically. Tester reports arrive with evidence instead of screenshots; the per-reply button also satisfies Google Play's requirement for a way to report AI-generated output.
- **"That wasn't me" on the wake-word overlay, zero-trust** *(v1.0; added 2026-09-07)* — One tap on a false "Hey Zynk" closes it and keeps the 3.2 s clip on the device as a negative example; real triggers the user confirms are kept as positives. Nothing leaves the phone: the personal verifier (a small classifier on the wake model's 96-dim embeddings, openWakeWord's "custom verifier" design) is trained on the phone from those clips. Explained in plain words on the toggle, off by default, clips listed and deletable in Settings, and the code path is small enough to audit. This is the fix for TV false triggers that no threshold change can deliver (21 firings in 32 min on 2026-09-07, scores 0.87–1.0, verified identical on desktop replay).
...
- **One dictation engine everywhere** *(post-v1.0; noted 2026-09-07)* — The Voice settings selector (Vosk offline vs. OpenAI Whisper) applies to the mic button only; the hands-free assistant session is Kotlin and always uses Vosk. Make the selector universal: the session records the utterance itself, ends it on silence by audio level, encodes WAV, posts to OpenAI's transcription endpoint with the stored key over the phone's own connection, and falls back to Vosk when there is no key or the call fails. About half a day of Kotlin plus phone testing; adds one to two seconds per turn and sends audio to OpenAI, which the modal already discloses.
