...
**Status (2026-09-09):** Parts 0–2 shipped on the `voice`/`memory` branches: Vosk offline dictation; "Hey Zynk" wake word running natively through the Android assistant role (openWakeWord models in a foreground service, defaults ON); timer, alarm and stopwatch handled locally by clock-app intents before any model call. Part 3 (voice memory query) works through the ordinary hands-free path, since a spoken question recalls memories like a typed one. The section below is kept as the original plan; where it names Porcupine, AlarmManager or "default OFF", the build did otherwise.
...
- ⚠️ **Known issue — Windows (KI-019, must fix before v1.0):** Vosk offline dictation does not work on Windows. The earlier diagnosis here — that `libvosk.dll` is "not yet bundled" — was wrong: `install.bat` already downloads the prebuilt `vosk-win64-0.3.45` SDK, and `START_ZYNKBOT.bat` already adds it to `PATH`. The real cause is that Vosk is **compiled out** on Windows by four gates (the `vosk` crate is a Linux-only dependency in `Cargo.toml`, the `build.rs` linker flags are Linux-only, `mod vosk_desktop` is Linux-only, and the recording commands return an error stub off-Linux). Two supporting defects: `install.bat` extracts only 2 of the 5 files the MinGW-based SDK needs, and the download has no retry and fails with a single easily-missed warning. OpenAI Whisper (cloud) is the current fallback, which means the offline-first guarantee does not hold on Windows. See KNOWN_ISSUES.md KI-019 for the full fix plan.
...
### ~~Pre-release gate — Wake-word 5-minute limit investigation~~ (resolved 2026-09-07)

No five-minute limit exists in the native pipeline; the observation came from the old WebView wake path, which needed the page awake. The only five-minute constant in the code today is the back-off window after repeated false triggers. Original note kept below.
...
**Payments, thinking ahead (Matt, 2026-09-13; not decided):** the first paid thing could ship with the first store version and be the *optional* hosted backup — a small monthly amount, price to be discussed, entirely optional and consistent with zero-trust because the backup is encrypted on the device before it leaves. Once snap-ins arrive: a seven-day free trial, then an in-app purchase (around $19.99 was the number floated). Both need a payment path — Stripe with a card kept on file — and the store rules decide where it can live: Google Play requires its own billing for in-app digital purchases (with the current US external-payment exceptions), so the Stripe path may be desktop-and-web while the phone uses Play Billing. No account is required for anything free; the purchase is the only contact with ContainAI. Open questions for when this is scheduled: price of the backup, whether a trial needs an account, and how a purchase on one device unlocks the others (the encrypted backup already carries a key; a signed licence file synced over ZynkSync is the obvious shape).

**Product tiers (decided 2026-09-07):** the core app is free and complete and includes the Kitchen kit; additional kits are one-time purchases (never a subscription to use your own kitchen); professional snap-ins are monthly subscriptions; Parenting/Elder modes and enterprise deployments sit above that. None of it touches the zero-trust design: a kit is data the app applies locally, and the only interaction with ContainAI is the purchase itself. See `labs/kits/KITS_DESIGN.md`.

### Remaining for v1.0

- **About me: tap a name under People, places and things to see the memories about it** (before 1.0, after beta3). Today the section is a list of names with counts and nothing is tappable; tags already filter the Memory Browser on tap, names should do the same (Matt, 2026-09-20).

- **Automated test coverage before any rebuild** *(Tier 1 — first job on `v1`; decided 2026-09-17)* — The suite is ~442 lines (63 Rust + 2 guards + 17 frontend) against ~49,000 lines of product code, and the sync layer, the chat pipeline, the LLM clients, memory extraction, backup/restore and all Kotlin have no tests; that is where most of the known issues came from. Target before the ZynkSync rebuild: two in-memory peers exercising pairing, key push, memory and history sync and tombstones; a chat request end to end with a fake model; every bug fix from here on ships with a test that failed before it. Why now and not earlier: the design churned through the beta (the voice path was rebuilt three times) and tests against a moving design are wasted; it has settled.
- **Tester-report fixes, first batch on `v1`** *(small; from the 0.9.6-beta2 reports, 2026-09-15–17)* — (a) KI-059 "Remembered on request" filter never populated; (b) KI-066 About me timeline accepted a future date — bound `event_date` to the day after it was said; (c) About me should state how many memories have no date instead of silently omitting them from the timeline; (d) the timer command should accept "set a time for …" (Vosk hears "time" for "timer"); (e) KI-061 timer set hands-free from a sleeping phone never fires (#29); (f) KI-060 an edited memory reverts after sync (#28); (g) KI-062 hands-free web search never runs when auto-execute is off (#26).
- **Basic vs advanced controls** *(v1.0; suggested by tester Mike 2026-09-15)* — System Controls has grown past what a first-time user needs. Basic mode: one first-run screen ("paste one API key, or use the computer's model") and a Settings page showing model, memory on/off, voice, sync and backup, with everything else behind "Advanced" (a switch, nothing removed). Belongs with the guided API-key onboarding and the backup-phrase first-run below — they are the same screen.
- **Hands-free threads split by inactivity** *(v1.0; tester report 2026-09-15)* — Twelve "Hey Zynk" exchanges on a road trip landed in one History thread because hands-free turns join the current thread by design. Start a new thread automatically when the last hands-free turn is more than an hour old (threshold to settle), and never split a thread the user has typed in within that window.
- ~~**Response streaming**~~ ✅ — token-by-token replies (`stream-token` events) have shipped for every backend; the native Android path streams into speech as well.
...
- **Extract Android Kotlin into a proper Tauri plugin** — `src-tauri/gen/android/` is generated output that `tauri android init` will overwrite, but it currently holds about 2,500 lines of hand-written production Kotlin across 13 files (count 2026-09-09): `MainActivity.kt` (879 lines — foreground service startup, ZynkbotPathsBridge, FolderPickerBridge, Vosk dictation bridge, screen-off transcript delivery), `WakeWordService.kt` (578 lines — ONNX wake-word loop, wake locks, chime, Vosk hand-off), `SyncForegroundService.kt` (59 lines), plus 12 `uses-permission` entries in the app manifest. Move this into a Tauri plugin (Kotlin plugin class + Rust command surface, with the plugin's own manifest fragment carrying the permissions) so the platform code becomes versioned source in our own tree rather than un-regenerable build output. Two payoffs: (1) `tauri android init` becomes safe to re-run, unblocking Tauri version upgrades that are currently risky enough to be documented as forbidden in `zynkbot_rust/CLAUDE.md`; (2) the hand-maintained surface shrinks to just the `MainActivity` *declaration* attributes (launch mode, `showWhenLocked`), which must stay in the app manifest because the activity belongs to the app — runtime calls like `setShowWhenLocked` / `setTurnScreenOn` can move into the plugin, which receives an Activity handle. Sequencing: do this **before the ZynkSync outbox**, and after one baseline test pass on the current build. The two projects share no surface — this one touches Kotlin, Gradle and the manifest; the outbox touches SQLite schema, triggers and the sync protocol — so running them sequentially means any breakage has exactly one possible cause. This one is also reversible (revert Kotlin files) where the outbox backfills UUIDs into a database holding years of real memory data. Do capture a baseline first, including screen-off wake word: relocating that code while still debugging it means every new failure has two possible causes.
- **Architecture decision (2026-09-17): one transport, three services.** `transport.rs` owns what is genuinely shared — identity, certificate, HTTPS server, pinned client, device registry, presence, peer verification — and ZynkSync, ZynkLink and ZChat are services on it, each owning its own tables and routes (done on `v1`, steps 1 and 3). Sync and Link are *pairing kinds* (the same user's other device; another user's device), not separate networks; Chat addresses any paired device, which is how notes-to-self work. Still to do: pairing itself into the transport (step 2), link pairing pinning certificates so link/chat routes return behind mTLS (closes the "ZynkLink mTLS cert exchange" item), the outbox as a transport facility used by sync and chat. A second carrier (Bluetooth, Wi-Fi Direct — see Advanced Networking) would sit under the transport without touching the services; not for 1.0.
...
- **Device identity survives a reinstall** *(first item after the beta; decided 2026-09-12; KI-050)* — A reinstall mints a new device id, so the phone reappears in every peer's ZynkSync list beside its old entry. Fix by carrying the device id (and the device name) inside the encrypted backup so a restore reclaims the old identity and the old peer rows simply keep working; no address heuristics in the pairing path. Same change should make the encrypted R2 backup a complete "new device" restore: today it carries memories and conversation history only (`backup.rs`, v2 payload); add the API keys and model settings (`.env`), the user and device identity, and the trained wake verifier, all under the existing backup key. Not the wake clips: they are raw material for training and diagnosis, capped at the 20 newest on the phone, and a verifier is what a restored phone needs, not the audio it was made from (decided 2026-09-12). The scenario (Matt, 2026-09-12): a phone lost or replaced far from home, with no paired device online to push keys from; the person enters the backup phrase on the new phone and gets everything back. Sizes are negligible (verifier 75 KB, `.env` under 1 KB). Until then testers delete both entries and pair again.
- **Uninstall should offer to remove user data** *(pre-1.0; noted 2026-09-13)* — On Windows the NSIS uninstaller correctly leaves `%LOCALAPPDATA%\zynkbot` alone, which means an uninstalled user keeps 5–6 GB of models and a database with no obvious way to reclaim them (only `uninstall.bat` knows). Add a checkbox to the uninstaller ("also delete my memories, keys and downloaded models") — Tauri's NSIS `deleteAppDataOnUninstall` targets the identifier folder, not ours, so it needs a custom uninstaller hook or a documented "Delete all data" in Settings. Linux `uninstall.sh` already asks.
- **Windows code-signing certificate** *(pre-1.0; noted 2026-09-13)* — The NSIS installer is unsigned, so every first run shows SmartScreen's "Windows protected your PC" and the tester has to click More info → Run anyway. Buy a code-signing certificate (an OV certificate is the cheaper route; EV skips SmartScreen reputation building) and add the signing step to the release workflow (`bundle.windows.certificateThumbprint` / `signCommand`). Until then the tester notes say what the warning looks like and how to get past it.
- **Address peers by identity, never by IP** *(with the ZynkSync outbox; noted 2026-09-13; KI-053)* — A device's identity is already its TLS certificate and device id; the IP is only where it was last seen, and on GrapheneOS that changes on every Wi-Fi connection (per-connection MAC randomisation). Today several notices are addressed by IP alone, which is how deleting a ghost entry reached the live phone at the same address (KI-053, patched by naming the target). Do it properly in the refactor: every peer-to-peer message names the target device id and the receiver verifies it; find current addresses by mDNS/NSD advertisement instead of remembering the last one. MAC addresses are not the answer: apps cannot read them on Android 10+, and randomised ones are not stable either.
- **Remove the in-service wake-word fallback** *(post-beta; decided 2026-09-12; KI-051)* — `WakeWordService` keeps a second, near-duplicate implementation of a hands-free turn for the case where the assistant service is not bound. It delivers a different behaviour silently and had drifted (three of the four wake-word bugs fixed on 2026-09-12 existed only there). Delete it; when `ZynkAssistantService.instance` is null, show "Hands-free needs Zynkbot set as the digital assistant" with the Default-apps shortcut, and log it. Do it together with the Kotlin-to-Tauri-plugin extraction, which restructures the same file.
...
- ~~**Vosk offline dictation**~~ ✅ — Linux and Android in v0.9.5-beta1; Windows in 0.9.6-beta1 (KI-019 fixed, verified 2026-09-11 and again on the 0.9.6-beta2 installer 2026-09-14).
- ~~**Windows offline dictation (KI-019)**~~ ✅ — the installer carries libvosk, its runtime DLLs and the model since 0.9.6-beta1.
- **Google Play readiness** *(deferred to the 1.0 candidate — Matt, 2026-09-17: nobody downloads from Play yet; allow a 2–3 week runway when the time comes — a personal developer account needs 12 opted-in testers on a closed track for 14 days before production, and the foreground-service declaration review takes days)* — Google Play is the short-term distribution target. Audit of the Sep 3 release build (2026-09-07) against current Play requirements; each item is a rejection if left as is:
  1. **App Bundle, not APK** — Play does not accept APK uploads for new apps. Produce an `.aab`; Play then splits per ABI, which takes the download from the 334 MB universal APK to roughly 120 MB (arm64 slice, small Vosk model included), under the 200 MB base-download cap. A larger Vosk model would break the cap, so it stays an optional in-app download rather than a bundled asset.
  2. ~~**16 KB memory-page alignment**~~ ✅ *(build33, 2026-09-07: vosk-android 0.3.75, onnxruntime-android 1.29.0; all five arm64 libs verified 0x4000 on a local NDK 30 build. The CI release workflow was still pinned to NDK 27 until 2026-09-09; it now uses NDK 30. Re-verify the first CI-built release with `llvm-readelf -l`.)* — mandatory since 2025-11-01 for apps targeting Android 15+ (we target 36). All five native libraries in the release APK are 4 KB-aligned: `libapp_lib.so` (build with NDK 30, already installed, or add `-Wl,-z,max-page-size=16384`), `libvosk.so` (vosk-android 0.3.47, a 2023 artifact — newer release or rebuild from source), `libonnxruntime.so` / `libonnxruntime4j_jni.so` (1.20.0 — upgrade), `libjnidispatch.so` (JNA, pulled in by Vosk — upgrade). Verify with `llvm-readelf -l`: every `LOAD` segment must show `Align 0x4000`.
  3. ~~**Drop `MANAGE_EXTERNAL_STORAGE`**~~ ✅ *(build34, 2026-09-07)* — The Knowledge Base on Android already lived in the app-private folder; all-files access only served the ZynkbotShare scan. Removed the permission and its prompt; documents now enter the KB through the system picker and a Kotlin copy (`copyToKnowledgeBase`), and ZynkbotShare through the existing "Add file" picker. Closes KI-015.
  4. **Foreground service declarations** — the `microphone` and `dataSync` service types each need a Play Console declaration with a written justification and a short demo video. Not code; prepare before upload.
  5. ~~**Remove `USE_FULL_SCREEN_INTENT`**~~ ✅ *(2026-09-07)* — Play restricts it to alarm and calling apps. The native assistant path answers without opening the app, so drop the permission, `ensureFullScreenIntentPermission()` in `MainActivity`, and the full-screen intent on the fallback notification in `WakeWordService` (keep the tap-to-open notification).
  6. ~~**Remove `READ_MEDIA_IMAGES`**~~ ✅ *(2026-09-07)* — photos come through the system photo picker, which needs no permission; the only remaining reference is the permission prompt in `MainActivity.FolderPickerBridge`.
  7. ~~**AI-generated content policy**~~ ✅ *(build36, 2026-09-07)* — a Report button under every reply and in System Controls builds a local problem report; confirm against the policy wording at submission.
  8. **Data safety form and privacy policy URL** — required for every listing. Declare the optional cloud calls (OpenAI, Anthropic, xAI, DuckDuckGo search), local-only storage, no accounts, no analytics.
  9. **Closed-testing track first** — upload the tester build here. On personal developer accounts created after 2023-11-13, Play requires 12 opted-in testers for 14 continuous days before production access; the tester lineup satisfies that.
  Then promote from closed testing to production.
...
- **Make the cautious window visible and explained** *(post-beta; noted 2026-09-13)* — After three empty firings the wake word enters "strict mode" for ten minutes and, since 2026-09-13, says "Were you asking me a question? I'm only answering questions for a few minutes" when it drops a statement. New users have no way to learn what that is, why it happened, or how to end it. The explanation to give (Matt, 2026-09-13): it exists to cope with a TV or background speech; while cautious it sends only things shaped like a question or a request and drops statements; outside it, a statement gets answered too. Wanted: a first-time explanation (one line in the wake-word settings text and in the onboarding card); a visible sign while it is active (a mark on the Z, a line in the wake-word notification); a "lift it now" control there; and Mike's answer to whether it should speak at all (asked in the tester email). The pause of a few seconds after a trigger and after the app is closed or the phone locked should be stated in the same place.
- **"Hey Zynk, stop" while the timer rings** *(post-v1.0; noted 2026-09-09)* — Today the alarm masks the wake word entirely (log 2026-09-09 20:37: no detection during the 20-second ring, one loud detection the instant it stopped) and there is no stop command anyway. Three parts, in order of value: a "stop the timer" / "dismiss" command sent to the clock app with `AlarmClock.ACTION_DISMISS_TIMER`; capture on the echo-cancelling audio source so the phone's own speaker is subtracted before the wake-word model hears it (an experiment; it changes the capture path for everything); and raised-voice clips in the verifier's training set, since a shouted "Hey Zynk" scored 0.40 against a 0.50 threshold. For v1.0 the answer is a tap.
...
- **Use the desktop's local model from the phone** *(v1.0; added 2026-09-07)* — Today the phone can call a cloud API or a custom endpoint; on-device GGUF models are compiled out on Android (`llm/mod.rs`), so this is the only local-first option for the phone. It cannot use the model already running on the user's desktop at home, which is the obvious local-first choice for anyone who does not want a model on the phone. The sync service already proxies `/api/ollama/*` to the desktop's Ollama on the LAN; turn that into a first-class backend: a "My desktop" entry in the model picker that resolves the paired desktop's address, health-checks it, streams through the existing mTLS channel, and falls back (with a clear message) when the desktop is away. Check what the current CUSTOM_API_URL path already covers before building. **Scope added 2026-09-13 (Matt):** the bridge today forwards only to Ollama (`localhost:11434`); the models a user downloads into Zynkbot's own model folder on the desktop (the GGUF files the desktop runs itself) are not reachable from the phone. For v1.0 the "My desktop" backend must serve those too, so a phone can use anything the desktop can run, Ollama or not. Matt also stated the intent that by v1.0 a phone can run a model placed in its own model folder; on-device GGUF for Android is currently under the v1.1/v1.2 "On-Device AI" section (compiled out today, CPU-only baseline first) — moving it into the v1.0 list is his call and is not made here.
- ~~**In-app "Report a problem"**~~ ✅ *(build36, 2026-09-07)* — One button (Settings, and on each reply) that assembles the last few hundred log lines, the build hash, device model, and the current thread into a text block the user copies into a GitHub issue. No server, nothing sent automatically. Tester reports arrive with evidence instead of screenshots; the per-reply button also satisfies Google Play's requirement for a way to report AI-generated output.
- **Personal wake-word verifier** *(partly shipped 2026-09-09)* — What exists: every trigger clip is kept on the phone with a `.real`/`.false` label from how the turn ended (a tap on the Z counts as false); a logistic-regression verifier over the classifier's embedding window, trained on the desktop from the owner's labelled clips against every false clip collected, ships as an asset and enforces at 0.5 (v2: 33 real clips at two distances vs 60 false; keeps 31/33, stops 56/60). The wake word works without a verifier; the verifier only reduces misfires (TV, silence, engine noise). What remains for v1.0: train on the phone or through the desktop app instead of a script; a Settings screen listing and deleting the clips; tester builds ship with enforcement off and collection on, so each user's verifier is their own. Nothing leaves the phone. Explained in plain words on the toggle, off by default, clips listed and deletable in Settings, and the code path is small enough to audit. This is the fix for TV false triggers that no threshold change can deliver (21 firings in 32 min on 2026-09-07, scores 0.87–1.0, verified identical on desktop replay). **Status 2026-09-14: enforcement suspended** (`owners` emptied) — in the field the v2 verifier rejected 13 of 17 of the owner's real triggers (scores mostly ≤ 0.02, a few 0.84–0.98; nothing in between). Likely cause, to be confirmed on exported clips: 1,536 of its 1,728 inputs are the 16 embeddings in time order, so it learned *where* in the window the phrase sat in the 33 training clips, and the fire point shifts by a frame or two between utterances. Next: score the real clips shifted ±1–3 frames to confirm; retrain on the shift-invariant mean/max part (192 dims) with shifted copies of every clip, on all real clips from both phones; report held-out numbers before re-enabling. Also: load the verifier (and its owners list) from the app's data folder instead of an APK asset, so a retrain is a file drop, not a release. **Decision 2026-09-15 (Matt): per-user custom builds do not scale — the product path is on-device training.** The phone already computes the 192 shift-invariant numbers at every trigger; save them next to each clip, ship a generic negative set as precomputed features (noise/TV plus consenting other voices), train a logistic regression in Kotlin when the counter reaches 30 ("Build my voice profile"), write the profile to the data folder (the loader for that shipped on `v1`, 14a29d9) and enforce on that phone; retrain as clips accumulate; nothing leaves the phone. Measured on 2026-09-15 with a second tester's 31 clips: the shipped v2 profile accepted 7/31 of his triggers (not a voice lock), the shift test lost 23/33 of the owner's clips at three frames, and the retrained mean/max form kept 87% of own triggers, stopped 93% of noise and let ~6% of the other voice through — a TV/other-people filter, to be described as such on the toggle, never as security. ~2 days.
- **Building on Windows from a clean clone fails** *(noted 2026-09-14 by the laptop session)* — `npm run tauri:build` stops with `resource path lib\vosk\libvosk.dll doesn't exist` because the Windows Vosk SDK is gitignored and fetched by `install.bat`, which is interactive and asks to disable antivirus. Add a short BUILDING-WINDOWS note (run `install.bat`, or fetch `vosk-win64-0.3.45.zip` into `lib\vosk`) or make the build script print that hint when the DLL is missing. Testers get binaries, so not urgent.
- **"Get Zynkbot for your computer" button on the phone** *(v1.0; added 2026-09-09)* — A Play Store install has no path to the desktop app. One button in the ZynkSync settings (not System Controls, so it reads as the first step to syncing with a computer rather than a push to install more), opening the desktop download page (zynkbot.com or the GitHub release); shown until the phone has an established sync connection to a non-Android device, then hidden. The desktop is where the local model, the knowledge base scanner and the larger features live, so the phone should point to it.
- **Backup phrase first-run** *(v1.0; added 2026-09-07)* — One plain-language screen at first launch: "this phrase unlocks your memories on every device you own", asked once, and the derived key travels with the API-key push (done 2026-09-07). Replaces the current warning that a second device shows after restore.
- **Kits mechanism + the Kitchen kit** *(v1.0; added 2026-09-07)* — Opt-in, data-only packages that give the memory system a visible job: standing instructions injected while the kit is on, a memory namespace, four opening questions, quick-prompt chips, a Kits section in System Controls (between Snap-ins and Knowledge Base, with future kits shown greyed), an onboarding card, and "forget everything from this kit". The Kitchen kit ships free: what you cooked, how it went, what you like and refuse, what to make tonight, how the week went. Design and the full kit JSON in `labs/kits/KITS_DESIGN.md`. Shares the event-date change with the memory report. Estimate two days plus content.
- ~~**"What do you know about me?" memory report**~~ ✅ *(build38–39, 2026-09-08)* — Shipped as "About me" in the Memory Manager: numbers, timeline by event date grouped by month, categories, tags (tap to filter the list), people/places/organisations/things, belief changes, kitchen, heard hands-free, tone, most recent; every line opens its memory. Not built: an "open questions" section (needs `temporal_status`) and a gaps section. Original note: The memory graph is visible but not yet useful to the user ("I stare at it wondering what good it is"). A generated report from the local data: topics and counts, recurring people and places, open questions, recent belief changes from the contradiction log, and gaps. Answers the transparency promise in one screen and gives the graph a job. Depends on the extractor emitting a real `event_date` (or null) — the single most valuable schema change identified by the 2026-09-07 analyst pass; without it "when" questions cannot be answered.
- **Secure Vault snap-in — credentials and secrets manager** *(High, near-term; added 2026-09-11)* — A dedicated encrypted store for API keys, passwords and other secrets, built on the snap-in architecture with isolated storage. **Why now:** testing on 2026-09-11 showed that a secret saved through the general `Remember:` pipeline is treated like any memory — indexed, pulled in by hybrid recall, and injected into the prompt sent to whichever model backend is active — a cross-backend exfiltration path that the sovereignty guarantee on storage does not cover. Secrets need a store walled off from memory; done well, this also makes Zynkbot a local-first password manager, a natural extension of the sovereign design. Requirements: **(1) Architectural isolation from the memory pipeline** — vault contents live in a separate encrypted store that is never indexed by semantic memory, never returned by RAG recall, and never placed in any LLM prompt context; this is the hard invariant, enforced at the storage layer, not by convention. **(2) Encryption at rest** — key derived from device credentials via the OS keychain / Android Keystore / Secure Enclave where available; biometric or PIN gate on unlock, with a configurable re-lock timeout. **(3) No hashing of stored secrets** — they must be recoverable in plaintext at use-time (hashing is one-way, suited only to verification); decryption happens in memory at the moment of use and the buffer is zeroed afterwards. **(4) Controlled egress** — when a secret is used (an API key for a backend call), the vault layer injects it directly into the request header; the model never sees it and it never appears in conversation logs or backups. **(5) Sync** — reuse the existing settings-push mechanism (partly implemented for API keys) to move vault entries between devices over ZynkLink's encrypted channel, and expose it in the UI rather than requiring manual entry or a device push. **(6) UX** — add/edit/delete entries, per-entry labels and backend associations, "reveal" gated behind biometric, and a local-only audit log of when each secret was accessed. **Docs and guard rails:** once the vault ships, document that `Remember:` must never be used for secrets (a warning is in FEATURES.md already); consider pattern detection that redirects the user to the vault when a secret-shaped string (API key prefixes, long high-entropy tokens) hits the memory pipeline.
...
- **Timeline: spoken and typed future dates (after beta3; possibly after 1.0).** Today the About me timeline lists only things that already happened: the memory pipeline drops any event date later than the day it was said (KI-066), because models invented dates for undated statements. Planned: keep the "no date unless the words state one" rule but allow dates ahead, so "I'm driving to Maryland on the 24th" lands on the 24th, spoken or typed; a tip on About me and in the docs saying a stated date goes on the timeline; tests that "next Tuesday", "on the 24th" and "September 24th" all resolve, dictated and typed. This is the hook for the Proton Calendar integration below (Matt, 2026-09-20).

- **"Open with Zynkbot" from the phone's Files app (after beta3).** Tapping a photo or PDF in the Files app should offer Zynkbot, and choosing it opens the app with the file attached to the composer. Needs a file-handling entry in the manifest, delivery to the page whether the app is running or cold-starting, and tests for both. For beta3 the in-app route ships instead: an Attach button in the file browser for the phone's own Zynkbot folder (Matt, 2026-09-20).

- **Contradiction dialog: dictate the explanation (after beta3).** The "resolve with an explanation" field takes typing only; it should offer the microphone like the composer does (Matt, 2026-09-20).
- **One voice (after beta3).** Replies are spoken by three different engines today: OpenAI's speech service for Read aloud and in-app spoken replies, Android's built-in engine for screen-off "Hey Zynk" answers, and Vosk is speech-to-text only. When an OpenAI key is present the screen-off path could use the same OpenAI voice; without one, Android's engine stays (Matt, 2026-09-20).

- **Wake word: noise-adaptive threshold (after beta3, Matt's idea 2026-09-21 — KI-069).** Distinct from the verifier-gating plan below: instead of (or alongside) checking who's speaking, raise the wake detector's own confidence bar only while the recent ambient noise floor is elevated, and relax it back to normal when the room is quiet. The thing this replaces, a fixed lockout or cooldown after a miss, was tried and rejected the same day it was proposed: Matt's own first attempt fails to wake the phone roughly a quarter of the time, so anything that blocks a retry after a miss — regardless of mechanism — breaks real use the same way strict mode did. Needs a working definition of "ambient noise floor" from the audio loop's own level readings (already logged every 2 s while listening) before there's anything to build.

- **Wake word: the owner's verifier as the TV gate (after beta3).** Strict mode is switched off in beta3 (`STRICT_MODE_ENABLED = false`): it locked the owner out after three ordinary misses. The verifier is a logistic regression over the wake detector's own 16×96 embedding window. The 09-14 one rejected 13 of 17 real wakes because (a) 1,536 of its 1,728 inputs are the window laid out frame by frame, so the same word said a little faster or fired a frame later looks different, and (b) all 33 positives came from two sessions on one day. Plan, in order: 1. collect 30+ real clips from ordinary use over several days at different distances and volumes (page-answered wakes count as of 2026-09-20), plus the `.false`-labelled clips as negatives; 2. change `labs/wake_verifier/train.py` to use only the 192 pooled features (per-dimension mean and max over the window; `WakeVerifier.score` already accepts that shape) and to read the `.real`/`.false` labels instead of date globs; 3. pick the threshold from cross-validation so at least 95% of real clips pass, and report how many false clips it stops at that threshold; 4. run it on the phone log-only for a week and count, from the log, how many answered wakes it would have rejected; enforce only if under 5%. 5. Once that holds, the real payoff Matt described 2026-09-21: separate the two jobs cleanly — loosen the wake *detector* (lower threshold/consecutive-hit requirement) so fewer real "Hey Zynk"s are missed, and let the *verifier* alone decide whether to actually answer, scored on the whole captured question, not just the 1.3 s trigger clip (more voice to work with, likely more accurate than what's scored today). A false wake still costs a chime and some listening either way; the verifier's job narrows to "was this worth answering," not "was this the wake word." Honest caveat carried over from `labs/wake_verifier/VOICE_LOCK_EXPERIMENT.md` (2026-09-13, never run): step 3 above proves the verifier recognises Matt; it says nothing about how well it rejects a television or another voice, which that file proposed testing and nobody has. Even unmeasured, gating responses this way is a strict improvement over today, where nothing checks the speaker before answering at all — but it is "answers mostly you," not a lock, until that second test happens. Two listening faults seen 2026-09-20 to look at alongside: a wake whose follow-up listening produced one word ("You") instead of the question, and one that ran 34 s on a YouTube video's audio (78 words) before the word-count gate rejected it.

- **Live dictation transcript, like a chat text box (after beta3, not important).** Today: tap the mic, speak, tap stop, the finished transcript appears once, then send. Wanted: text appears as you speak, with a level/waveform indicator, editable before sending — Matt described a text-input app where it works that way (2026-09-20). Partly built already for Vosk: the Kotlin bridge already emits live partial text on every phrase (`window.__voskPartial`, `VoskBridge.onPartialResult`), but the composer never subscribes to it — it is cleared and ignored in `useVoiceInput.js`. Wiring that up gets live text for offline (Vosk) dictation. OpenAI Whisper dictation records the whole clip and sends it once you stop, so it has no partial text to show without switching to a streaming transcription endpoint — a separate, larger change.

### Camera photos: save to the gallery too? (open question, Matt 2026-09-20)

Right now Zynkbot's own in-app camera button (the one in the composer) only keeps the photo long enough to attach it to the message — it lives in a temp cache file, never saved anywhere permanent, and is not in the phone's gallery or the Zynkbot share folder. Matt, while testing B4: a photo taken this way should probably end up in both places, so it's easy to find and easy to share again later.

A broader, separate idea also worth naming explicitly rather than defaulting into: should *every* photo taken with the phone's own Camera app also land in the Zynkbot share folder automatically? That's a bigger, standing decision (every photo becomes visible to linked devices without an explicit share step each time) and deserves its own yes/no, not something to bundle in by default.
...
### Web Search Quality (after v1.0)

The current web search (DuckDuckGo, top 3 pages fetched for content) is a first pass, not a finished feature — real work is planned here to bring it closer to what Claude and OpenAI's own search does (better result ranking, more reliable content extraction, maybe a better search backend). Fold this question into that work rather than deciding it in isolation:
...
- **Brainstorm mode** *(post-v1.0; noted 2026-09-07)* — A conversation mode that makes Zynkbot work the way a good design session with an AI works, drawn from how Matt used Claude on 2026-09-07: (1) offer directions the user would not have thought of, anchored to what already exists (the user's memories and documents), then let the user prune; (2) argue against its own recommendation on request, giving the strongest case for the other side; (3) ask for the constraints first (time, money, "must ship this week") before proposing a plan; (4) when something went wrong, ask for the evidence (log, screenshot, the actual text) before offering a theory. Implemented as a standing-instructions block plus two or three quick prompts ("Argue the other side", "What are my constraints?", "Show me the evidence first"), so it can ship as a kit. Pairs with the memory report: brainstorming from what Zynkbot already knows about the user is the point.
- ~~**One dictation engine everywhere**~~ ✅ *(build40, 2026-09-08)* — Done as described: OpenAiDictation.kt, Vosk fallback, selector carried to Kotlin. Original note: The Voice settings selector (Vosk offline vs. OpenAI Whisper) applies to the mic button only; the hands-free assistant session is Kotlin and always uses Vosk. Make the selector universal: the session records the utterance itself, ends it on silence by audio level, encodes WAV, posts to OpenAI's transcription endpoint with the stored key over the phone's own connection, and falls back to Vosk when there is no key or the call fails. About half a day of Kotlin plus phone testing; adds one to two seconds per turn and sends audio to OpenAI, which the modal already discloses.
- **Optional larger Vosk model** *(decision point: after the beta, shortly before release; noted 2026-09-07, timing set 2026-09-08)* — Keep the 68 MB small model as the default (works, and Play's base-download cap leaves no room for more). Offer the 128 MB en-us-0.22-lgraph model as a one-tap download in Voice settings for users who want better offline accuracy without sending audio anywhere; the unpack-from-APK path already has a download fallback to reuse. No Vosk model produces punctuation; that is Whisper's job.
- **Personal wake phrase** *(post-v1.0, with Elder Mode; product decision 2026-09-09)* — Everyone starts with "Hey Zynk". A custom phrase ("Hey Marvin") is a paid one-time feature: the phrase model is trained on the user's own computer from synthetic samples of the phrase (the way `hey_zynk` was), synced to the phone, and paired with the personal verifier trained from the user's own voice. Two models, two jobs: the phrase model decides "was the phrase said", the verifier decides "was it this person"; a generic phrase still needs the first, which is why the phrase is the paid part and the verifier stays free. Offer the first hundred beta users a free custom phrase in return for testing. Prerequisites: the verifier training moved from a script into the desktop app; the openWakeWord training pipeline packaged to run there.
- **Pin the built-in voice as a character** *(back burner; Matt to decide later whether the default voice is acceptable; noted 2026-09-08)* — The phone's default offline English voice is distinctive ("Granny Z"). If it becomes a deliberate character rather than an accident, pin that voice by name in Voice settings so it is the same on every device and does not change with an OS update, keep OpenAI TTS as the smooth alternative, and say so in the tester notes.
- **News kit** *(post-v1.0; noted 2026-09-08)* — A one-time kit, like the others: the user names their sources as ordinary RSS feeds, Zynkbot fetches only those, directly, and produces a morning summary with the selected model: the stories most sources agree matter, the ones only one or two ran, and how different outlets frame the same event (grouped by story from the feeds alone, cruder than a bias service but private). Memory adds continuity ("you followed the Vermont flooding story last week; here is what changed"). Needs a feed fetcher/parser in Rust, a story-clustering step, and a summary prompt. A Ground News snap-in was the original idea; it waits on Ground News offering a public API, which as of this note they do not.
- **Photo references in memories** *(post-v1.0; noted 2026-09-07)* — A memory can point at a photo on the device instead of holding it. Use case from the kitchen kit: a daily screenshot of the bathroom scale. The app already takes and attaches photos; when one is attached with a vision-capable model in use, extract the fact into the memory text ("scale read 182.4 lb, 2026-09-07") and store only a link (content URI / path) to the image in the user's own photo album. The number survives even if the link does not. Broken links are tolerated by design: the Memory Manager shows "photo no longer available" and nothing else happens; no copying, no thumbnails, no image store, so the database stays small. Needs a nullable `media_ref` field on memories and a "view photo" affordance in the Memory Manager; the sync layer carries the reference but never the image.
- **Kit purchases** *(post-v1.0; noted 2026-09-07)* — Additional kits as one-time purchases: catalogue shown in the app, purchase on zynkbot.com, unlock by a small signed licence file verified offline (no account, no server call from the app). Desktop shows buy links; the Android build's buy link depends on Google Play's external-purchase policy at submission time (Play Billing rules vs. the *Epic v. Google* order) — worst case it says "available at zynkbot.com". Requires the sync tombstones (KI-028) before the "forget it and start again" promise is made to paying users.
- **Multi-step web search** *(post-v1.0; noted 2026-09-07)* — Today a question gets at most one web search: the model asks for a query, Zynkbot runs it, and the answer is synthesised from that one result set. Questions that need a second lookup (a fact that depends on the first answer, a comparison across two things, a date to resolve before the real search) come back thin or wrong. Fix: let the synthesis step request a follow-up search, run it, and fold the new results in, under a small budget (e.g. up to 3 searches per question, each shown in the transcript the way the first one is now). Auto-run the follow-ups when the first search was auto-run (hands-free); otherwise show each proposed query for confirmation as today. Working well enough for v1.0; schedule after.
...
- ~~**Android Storage Access Framework (SAF) migration**~~ *(superseded 2026-09-07)* — `MANAGE_EXTERNAL_STORAGE` is gone; files enter ZynkbotShare and the Knowledge Base through the system picker (see the Google Play block, item 3). A persistable folder grant remains a possible later enhancement. Original note: the build relied on `MANAGE_EXTERNAL_STORAGE` to see files placed in `Downloads/ZynkbotShare/` by other apps. Google Play restricts this permission heavily. Migration to SAF is the path to Play Store compliance. **Superseded again 2026-09-19:** the share folder is Zynkbot's own storage location (`ZynkShareProvider`), so any app can put files in it and Zynkbot owns them; no picker, no permission, no SAF.
  ...
  **Interim (historical):** `MANAGE_EXTERNAL_STORAGE` was declared and requested at first launch until 2026-09-07. KI-015 records the resolution.
...
- **Sync Conflict Viewer** — UI to review past conflict resolutions.
- **Selective Device Sync** — choose which paired devices receive which namespaces.
- **Conversation pins and user-chosen titles follow the thread across devices (after beta3).** Today `pinned` never leaves the device it was set on, and a synced session keeps its own auto-generated title even after another device renames it — the receiver only fills in a title when it has none (`zynksync.rs` conversation push/receive). Found 2026-09-20: after the OnePlus was wiped and re-synced, a thread Matt had renamed and pinned on the desktop came back unpinned with its original auto-title. Fix: carry `pinned` in the session push, and a "named by the user" flag so a chosen title wins over an auto one on receipt. Do alongside the sync refactor (pairing/outbox work), with harness tests.
...
### Visual Design — Revisit the Theme (no priority set)

- **Matt, 2026-09-20:** "I sometimes wonder if I should rethink the vampire color scheme... it is kind of ugly... it looks kind of basic." Prompted by using an app with a more polished look (this Claude Code / claude.ai interface) side by side with Zynkbot's current Dracula-derived dark purple theme.
- **Constraint to keep:** plain, spelled-out labels over cryptic icons — "I don't use symbols... it always pisses me off when I have to pause and figure out what symbols mean what." Worth reconciling before any redesign: the current UI already leans on emoji-prefixed labels throughout (🔗 Linked Devices, 📁 My shared folder, 📂 Open folder, 📎 Attach, ⚙ Indexing, ✕ Delete, 🎙️/🧠 section markers, and more added this session with Matt's own approval). A redesign either keeps the emoji as visual anchors next to full words (current pattern) or moves away from them — worth deciding explicitly rather than by default, since it directly touches what he said he wants.
- Not scoped: no palette, layout, or component decisions made yet. Needs its own pass — likely worth a proper design system reference the way other Anthropic-built interfaces use one, rather than incremental color tweaks.
