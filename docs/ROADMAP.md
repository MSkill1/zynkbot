...
- **Vosk offline dictation** — Linux (cpal + vosk crate) and Android (Kotlin bridge) shipped in v0.9.5-beta1. **Windows still outstanding — see next item.** Wake-word, timer, and voice memory query deferred to a later v0.9.x release.
- **Windows offline dictation (KI-019)** — Windows has no offline dictation today; it falls back to OpenAI Whisper, so the offline-first guarantee does not hold on Windows. Vosk is not unavailable on Windows, it is compiled out: the `vosk` crate is scoped to `cfg(target_os = "linux")` in `Cargo.toml`, `build.rs` gates the linker flags to Linux, `mod vosk_desktop` is Linux-only, and the recording commands return an error stub off-Linux. Fix: widen those four gates to `cfg(any(target_os = "linux", target_os = "windows"))`, add a Windows `rustc-link-search` pointing at `lib/vosk`, and make `install.bat` extract all five files from the MinGW-based SDK (it currently copies 2 of 5, omitting `libstdc++-6.dll`, `libwinpthread-1.dll`, `libgcc_s_seh-1.dll`). Unverified risk: linking a MinGW import library into an MSVC build; fallback is runtime `LoadLibrary` binding. Must ship before v1.0.
- **Google Play readiness** *(Tier 1 — gates the tester rollout)* — Google Play is the short-term distribution target. Audit of the Sep 3 release build (2026-09-07) against current Play requirements; each item is a rejection if left as is:
